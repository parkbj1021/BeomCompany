...
```
💡 beom-[DOMAIN] — AI가 분석한 이번 세션 핵심 학습:
...
ALL_DIRS=($(ls -d "$BASE_PATH/beom-${DOMAIN}-v"* 2>/dev/null | sort -V))
LATEST_DIR="${ALL_DIRS[-1]}"
CURRENT_VERSION=$(cat "$LATEST_DIR/VERSION" 2>/dev/null || echo "1")
NEXT_VERSION=$((CURRENT_VERSION + 1))
NEW_DIR="$BASE_PATH/beom-${DOMAIN}-v${NEXT_VERSION}"
...
Edit 도구로:
- `"./plugins/beom-[DOMAIN]-v[CURRENT]"` → `"./plugins/beom-[DOMAIN]-v[NEXT]"`
...
```
✅ beom-[DOMAIN] 버전업 완료
📦 현재 버전: beom-[DOMAIN]-v[NEXT] (VERSION=[NEXT])
📦 보관 버전: beom-[DOMAIN]-v[CURRENT] (직전)
🗑️ 삭제됨: [삭제된 버전들]
